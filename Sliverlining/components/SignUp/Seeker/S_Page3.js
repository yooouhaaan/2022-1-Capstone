...
export default function S_Page3({navigation}) {
  ...
  return (
    ...
    <ScrollView style = {{height : WidthAndHeight.windowHeight*0.43, padding : 10, borderWidth : 1, borderColor : theme.mColor, width : WidthAndHeight.windowWidth*0.8}}>
          ...
          <View style = {styles.jobs}>
          <Button title = "고객상담/영업/리서치" color = {clicked[6] ? 'gray' : theme.mColor} onPress = {() => setJob(6)}></Button><Text>{"\t"}</Text>
          ...
          <View style = {styles.jobs}>
            ...
            <Button title = "교육/강사" color = {clicked[9] ? 'gray' : theme.mColor} onPress = {() => setJob(9)}></Button><Text>{"\t"}</Text>
          </View>
